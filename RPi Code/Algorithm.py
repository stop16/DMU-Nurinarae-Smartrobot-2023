	...
	while True:
		...
		if len(contours) != 0:
			...
			if cv2.contourArea(contour) > 300:
				...
				rame = cv2.rectangle(frame,(x,y),(x+w,y+h),(0,255,0),2)
				cv2.putText(frame, "red", (x,y), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,0))
		...
		if len(contours) != 0:
			...
			if cv2.contourArea(contour) > 300:
				...
				cv2.putText(frame, "red", (x,y), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255,0,0))
		...
		if len(contours) != 0:
			...
			if cv2.contourArea(contour) > 300:
				...
				cv2.putText(frame, "red", (x,y), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,255,255))
...
def get_dir(vm,y):
    if vm[y][0] != color[3] and vm[y][0] != color[4]:
        return 0
    elif vm[y][1] != color[3] and vm[y][1] != color[4]:
        return 1
    else:
        return 2
...
5. 같은 색상 큐브가 여러개일 경우 -> 겹친 기둥 index 확보 -> 기둥과 남는 색 비교 -> 안겹치는 index의 큐브를 남는 색상으로 변경
6. TBD
"""
def auto_correction(VM_Pillar, VM_Cube, errflag):
    ...
    if errflag[5] == 1 and get_num(VM_Pillar,color[3]) > 1:
        ...
        if VM_Cube[tmpxy[0]][tmpxy[1]] != color[4]:
            ...
            VM_Pillar[tmpxy[0]][tmpxy[1]] = color[i]
    if errflag[4] == 1:
        red_count = get_num(VM_Cube,color[0])
        green_count = get_num(VM_Cube,color[1])
        blue_count = get_num(VM_Cube,color[2])
        big = max(red_count,green_count,blue_count)
        small = min(red_count,green_count,blue_count)
        biggest = None
        smallest = None
        if red_count == big:
            biggest = color[0]
        elif green_count == big:
            smallest = color[1]
        elif blue_count == big:
            biggest = color[2]
        if red_count == small:
            smallest = color[0]
        elif green_count == small:
            biggest = color[1]
        elif blue_count == small:
            smallest = color[2]
        for i in range(0,4):
            for j in range(0,2):
                if VM_Pillar[i][j] != smallest and VM_Cube[i][j] == biggest:
                    VM_Cube[i][j] == smallest
...
"""
def robot_move():
    global shared_cube, shared_pillar, VM_Cube, VM_Pillar, cube_storage, robot_dir, color
    cube = 0
    pillar = 0
    current_dir = 0
    current_loc = 4
    ...
    sync_event.wait()
    with shared_variables_lock:
        VM_Pillar[current_loc][0] = shared_pillar
        VM_Cube[current_loc][0] = shared_cube
        shared_pillar = 'None'
        shared_cube = 'None'
    if VM_Pillar[current_loc][0] == 'None':
        ...
        sync_event.wait()
        with shared_variables_lock:
            VM_Pillar[current_loc][1] = shared_pillar
            VM_Cube[current_loc][1] = shared_cube
            shared_pillar = 'None'
            shared_cube = 'None'
    #1-1
...
robot_thread = threading.Thread(target = robot_move)
